...
RUN cd /tmp/                                                                           && \
    apk --no-cache update                                                              && \
    apk --no-cache add python python3                                                         && \
    apk --no-cache --virtual guzo-dependencies add alpine-sdk ncurses-dev python-dev python3-dev  && \
    git clone https://github.com/vim/vim.git                                           && \
    cd vim                                                                             && \
    ./configure --prefix=/usr --enable-pythoninterp --enable-python3interp                                   && \
    make install                                                                       && \
    apk del alpine-sdk ncurses-dev python-dev python3-dev guzo-dependencies                        && \
    rm -rf /tmp/vim
